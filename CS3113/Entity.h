...
private:
    ...
    Vector2 mMovement;
    Vector2 mVelocity;
    Vector2 mAcceleration; // prob important 
    ...
    float mDragFactor = 0.9f;

    bool isColliding(Entity *other) const; // this 
...
public:
    ...
    void accelerateUp() { mAcceleration.y = -60.0f;  mDirection = UP; }
    void accelerateDown() { mAcceleration.y = 100.0f;  mDirection = DOWN;}
    void accelerateLeft() { mAcceleration.x = -50.0f;  mDirection = LEFT;}
    void accelerateRight() { mAcceleration.x = 50.0f;  mDirection = RIGHT;}
    ...
    // "When the player releases the movement key, the ship should drift briefly before coming to a stop."" right now it just keeps going, drag to slow it down
    void setDragFactor( float newDrag) { mDragFactor = newDrag; }
    float getDragFactor ()  {return mDragFactor; }
