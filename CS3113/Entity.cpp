...
//Vector2Distance(mPosition, target->getPosition())
...
//switch state if idle check distance if its following switch to lurping fi its idle then walking 
    ...
    int collisionCheckCount, Entity* blocks, int blockCount)
{
    ...
    if (mAIType != LERPER)
    {
        // no movement edit in this project mVelocity.x = mMovement.x * mSpeed;

        // This determines the movement for the Bee
        mVelocity.x += mAcceleration.x * deltaTime;
        mVelocity.y += mAcceleration.y * deltaTime;

        // to implement the drag to slow down whichever way its going

        mVelocity.x *= mDragFactor;
        mVelocity.y *= mDragFactor;
        ...
        // this actualy sets the position for the Bee, check collision is prob important 
